<template>
  <div class="my-friends" v-if="isLoading === true">
    <template v-if="$friendsStore.receivedRequests.length !== 0">
      <template v-for="friend in $friendsStore.receivedRequests" :key="friend.id">
        <FriendCard
    ...
    <span class="text-4xl mt-7" v-else>No received requests</span>
  </div>
  <img 
    v-else
